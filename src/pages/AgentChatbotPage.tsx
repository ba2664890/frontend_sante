...
const parseBold = (text: string, isAgent: boolean = true) => {
  const boldRegex = /(\*\*[^*]+\*\*)/g;
...
const parseInlineContent = (text: string, isAgent: boolean = true) => {
              ...
              if (listMatch) {
                ...
                return (
                  <div key={lIdx} className="flex gap-2 items-start pl-2">
                    ...
                    <span className="text-[#3e4949] font-medium">{parseInlineContent(itemContent, true)}</span>
                  </div>
                );
              }
              return (
                <p key={lIdx} className="text-[#3e4949] leading-relaxed">
                  {parseInlineContent(line, true)}
                </p>
              );
            })}
          </div>
